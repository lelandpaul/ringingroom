...
def on_join(json):
    ...
    if not json["anonymous_user"]:
        try:
            user_id = jwt.decode(
                json["user_token"], app.config["SECRET_KEY"], algorithms=["HS256"]
            )["id"]
    ...
    if not tower.host_present():
        ...
        emit(
            "s_host_mode",
            {"tower_id": tower_id, "new_mode": False},
            broadcast=True,
            include_self=True,
            room=tower_id,
        )
...
def on_kick_user(json):
    ...
    emit(
        "s_user_left",
        {"user_id": user_id, "username": user.username, "kicked": True},
        broadcast=True,
        include_self=True,
        room=tower_id,
    )
...
def on_assign_user(json):
    ...
    emit(
        "s_assign_user", json, broadcast=True, include_self=True, room=json["tower_id"]
    )
...
def on_silent_swap(event_dict):
    ...
    emit(
        "s_silent_swap",
        {"new_bell_state": cur_tower.bell_state[cur_bell - 1], "who_swapped": cur_bell},
        broadcast=True,
        include_self=True,
        room=tower_id,
    )
...
def on_host_mode(json):
    ...
    emit("s_host_mode", json, broadcast=True, include_self=False, room=tower_id)
