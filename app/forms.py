...
class LoginForm(FlaskForm):
    username = StringField(
        ...
        validators=[
            DataRequired(),
            Email(
                message="Please use your email address to log in, not your username."
            ),
        ],
    )
...
class RegistrationForm(FlaskForm):
    ...
    accept_privacy = BooleanField(
        ...
        validators=[
            DataRequired(message="Please accept our Privacy Policy to continue.")
        ],
    )
    ...
    def validate_email(self, email):
        ...
        if user is not None:
            raise ValidationError(
                "There is already a username associated with that email address."
            )
...
class UserSettingsForm(FlaskForm):
    ...
    new_password2 = PasswordField(
        "Repeat New Password",
        validators=[EqualTo("new_password"), RequiredIf("new_password")],
    )
...
class UserDeleteForm(FlaskForm):
    delete_password = PasswordField(
        "Enter your password to delete", validators=[DataRequired("delete")]
    )
...
class TowerSettingsForm(FlaskForm):
    ...
    additional_sizes_enabled = BooleanField(
        "Additional Sizes Permitted", validators=[Optional()]
    )
    muffled = SelectField(
        "Muffled Tower Bells",
        choices=[
