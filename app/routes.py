...
def load_toasts(modal):
    return [
        render_template("news/" + f, modal_id=i, modal=modal)
        for (i, f) in enumerate(os.listdir("app/templates/" + "news/"))
        if not f.startswith(".")
    ]
...
def tower(tower_id, decorator=None):
    ...
    user_token = "" if current_user.is_anonymous else current_user.get_token()
    ...
    return render_template(
        ...
        user_settings=Config.DEFAULT_SETTINGS
        if current_user.is_anonymous
        else current_user.get_settings_with_defaults(),
        server_ip=get_server_ip(tower_id),
...
@app.route("/about")
def about():
    return render_template("about.html", version=Config.RR_VERSION)
...
def donate():
    ...
    return render_template(
        "donate.html", patrons=patrons.values(), is_a_patron=is_a_patron
    )
...
def apple_app_site():
    return send_from_directory(
        app.static_folder, "./.well-known/apple-app-site-association"
    )
...
def login():
    ...
    if login_form.validate_on_submit():
        user = User.query.filter_by(
            email=login_form.username.data.lower().strip()
        ).first()
...
def tower_settings(tower_id):
    ...
    if form.validate_on_submit():
        ...
        if tower.wheatley.enabled != wheatley_enabled:
            ...
            if wheatley_enabled:
                ...
                emit_socket(
                    "s_user_entered",
                    {"username": WHEATLEY_USER_NAME, "user_id": WHEATLEY_USER_ID},
                )
        ...
        if form.remove_host.data:
            ...
            if host == tower_db.creator:
                form.add_host.errors.append(
                    "Cannot remove tower creator from host list."
                )
            else:
    ...
    form.muffled.data = (
        "Toll"
        if tower.fully_muffled and tower.half_muffled
        else "Full"
        if tower.fully_muffled
        else "Half"
        if tower.half_muffled
        else "Open"
    )
