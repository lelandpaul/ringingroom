...
The integration of Wheatley into Ringing Room has added a number of extra SocketIO signals, used
for keeping Wheatley in sync with the rest of Ringing Room. Some of these signals have custom types
(`RowGen` and `Settings`), which are described in detail below the table.
...
| `s_wheatley_setting`         | `Settings`                            | (from the server) tells Wheatley to change one of its settings, and for all the clients to update their views of that setting. To prevent rubber-banding of controls, this signal will **not** be sent to the client that emitted the corresponding `c_wheatley_setting` signal. |
...
| `s_wheatley_is_ringing`      | `{is_ringing: Bool}`                  | broadcast from the server after Wheatley sends `c_wheatley_is_ringing`                                                                                                                                                                                                           |
